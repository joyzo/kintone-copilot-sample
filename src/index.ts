...
async function main() {
  ...
  try {
    ...
    const createResult = await kintoneService.createRecord("1", {
      ...
      description: {
        value:
          "TypeScriptとREST APIクライアントを使って作成されたレコードです。",
      },
    });
    ...
    const updateResult = await kintoneService.updateRecord(
      "1",
      createResult.id,
      {
    ...
    const records = await kintoneService.getRecords(
      "1",
      undefined,
      undefined,
      true
    ...
    console.log(
      `取得したレコード数: ${records.records.length}/${records.totalCount}`
    );
